...
<#
  ...
  Config (optional):
    create-pos-terminals.config.json in RunDir, or pass -ConfigPath.

  Example config:
  {
    "SqlServer": "localhost\\SQLEXPRESS",
    "Database": "BepozVenueDb",
    "VenueId": 1,
    "StoreId": 1,
    "TemplateWorkstationId": 10,
    "Count": 3,
    "NamePrefix": "POS",
    "StartNumber": 1,
    "Padding": 2,
...
function Get-RunDir {
  if ($env:BEPoz_TOOLKIT_RUNDIR -and (Test-Path -LiteralPath $env:BEPoz_TOOLKIT_RUNDIR)) {
    return $env:BEPoz_TOOLKIT_RUNDIR
  }
  # Fallback: script root (still write Report.json somewhere)
  return $PSScriptRoot
}
...
function Ensure-STAIfInteractive {
  param([bool]$NeedUi)

  if (-not $NeedUi) { return }

  try {
    $state = [System.Threading.Thread]::CurrentThread.ApartmentState
    if ($state -ne [System.Threading.ApartmentState]::STA) {
      # Relaunch ourselves in STA (best-effort)
      Write-Log -Level 'WARN' -Message "Current thread is $state; relaunching in STA for WinForms UI."
    ...
    Write-Log -Level 'WARN' -Message ("Could not enforce STA: " + $_.Exception.Message)
  }
}
...
function Get-DefaultSqlServerGuess {
  # Prefer default MSSQLSERVER, else first named instance, else localhost.
  try {
    ...
    if ($named) {
      $inst = $named.Name.Substring(6) # MSSQL$INST
...
"@
  ...
  foreach ($r in $dt.Rows) {
    ...
    # Exclude identity & computed; also exclude rowversion/timestamp (system_type_id 189)
...
function Set-CommandParamValue {
  ...
  if ($null -eq $Value) {
    $Command.Parameters[$ParamName].Value = [DBNull]::Value
  } else {
...
$report = @{
  ...
  plan = @{ count = $null; namePrefix = $null; startNumber = $null; padding = $null; ipAddressInt = 0; copyExportCodes = $false }
  dryRun = $true
  confirmed = $false
  results = @()
  logs = $null
...
try {
  ...
  Ensure-STAIfInteractive -NeedUi:$interactive
  ...
  if ($cfg) {
    ...
    $dryRun = -not (Show-YesNo -Title "Dry Run?" -Message "Run in DRY RUN mode first?`n`nYes = plan only (recommended)`nNo = will insert after confirmation")
    # If user clicked No on "Dry Run?", then dryRun becomes $false. Our logic above inverts, so fix:
    $dryRun = -not $dryRun
    # Explanation: Show-YesNo returns $true if Yes; we want dryRun=$true on Yes.
    # The two negations above set it right.
  ...
  try {
    ...
    if (-not $venueId) {
      ...
      $venues = foreach ($r in $venuesDt.Rows) {
        [pscustomobject]@{ VenueID = [int]$r['VenueID']; Name = [string]$r['Name'] }
      }
    ...
    # Store selection (filtered by venue)
    if (-not $storeId) {
      ...
      $stores = foreach ($r in $storesDt.Rows) {
        [pscustomobject]@{ StoreID = [int]$r['StoreID']; Name = [string]$r['Name'] }
      }
    ...
    # Template workstation selection
    if (-not $templateWsId) {
      if (-not $interactive) { throw "TemplateWorkstationId is required in config for non-interactive runs." }

      $wsDt = Invoke-SqlQuery -Connection $conn -Sql @"
SELECT WorkstationID, Name
FROM dbo.Workstation
WHERE StoreID=@s
ORDER BY WorkstationID;
"@ -Parameters @{ "@s" = $storeId }

      $workstations = foreach ($r in $wsDt.Rows) {
        [pscustomobject]@{ WorkstationID = [int]$r['WorkstationID']; Name = [string]$r['Name'] }
      }
    ...
    # Determine insertable columns and prep insert command
    ...
    # Preload existing names in store (for uniqueness)
    ...
    function Format-PosName {
      param([string]$Prefix, [int]$Number, [int]$Pad)
      $n = if ($Pad -gt 0) { $Number.ToString().PadLeft($Pad, '0') } else { $Number.ToString() }
      return ("{0} {1}" -f $Prefix.Trim(), $n).Trim()
    }

    # Build the name list (skip collisions)
    ...
    foreach ($item in $namesToCreate) {
      Write-Log ("  - {0}" -f $item.Name)
    }

    # Always require explicit confirmation before changes
    if (-not $dryRun) {
      if ($interactive) {
        ...
        $confirmed = Show-YesNo -Title "Confirm create POS terminals" -Message $msg
        if (-not $confirmed) {
        ...
        # Non-interactive: require Confirm=true in config
        if (-not $confirmFlag) {
